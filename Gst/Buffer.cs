...
namespace Gst {
	...
	public partial class Buffer : Gst.MiniObject {
		...
		public static unsafe ulong GetPts(IntPtr raw)
		{
			ulong* raw_ptr = (ulong*)(((byte*)raw) + abi_info.GetFieldOffset("pts"));
			return (*raw_ptr);
		}
        ...
        [DllImport(DLL.GStreamer, CallingConvention = CallingConvention.Cdecl)]
        static extern UIntPtr gst_buffer_fill(IntPtr raw, UIntPtr offset, IntPtr src, UIntPtr n_length);
		...
		public ulong Fill(ulong offset, IntPtr src, int srcLength)
        {
            UIntPtr raw_ret = gst_buffer_fill(Handle, new UIntPtr(offset), src, new UIntPtr((ulong)(src == IntPtr.Zero ? 0 : srcLength)));
            ulong ret = (ulong)raw_ret;
            return ret;
        }
		...
		[DllImport(DLL.GStreamer, CallingConvention = CallingConvention.Cdecl)]
		public static extern bool gst_buffer_map(IntPtr raw, IntPtr info, int flags);
		...
		public static bool Map(IntPtr raw, out Gst.MapInfo info, Gst.MapFlags flags)
		{
			IntPtr native_info = Marshal.AllocHGlobal(Marshal.SizeOf(typeof(Gst.MapInfo)));
			bool raw_ret = gst_buffer_map(raw, native_info, (int)flags);
			bool ret = raw_ret;
			info = Gst.MapInfo.New(native_info);
			Marshal.FreeHGlobal(native_info);
			return ret;
		...
		public static void Unmap(IntPtr raw, Gst.MapInfo info)
		{
			IntPtr native_info = GLib.Marshaller.StructureToPtrAlloc(info);
			gst_buffer_unmap(raw, native_info);
			Marshal.FreeHGlobal(native_info);
		}
        ...
        {
            Raw = gst_buffer_new_and_alloc(size);
        }

		static IntPtr gst_buffer_new_and_alloc(uint size)
        {
            return gst_buffer_new_allocate(IntPtr.Zero, new UIntPtr(size), IntPtr.Zero);
        }
